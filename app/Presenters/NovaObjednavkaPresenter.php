...
class NovaObjednavkaPresenter extends ObjednavkyBasePresenter
{
    ...
    protected function createComponentObjednavkyForm1(): Form
    {
        ...
        $form->addText('popis', 'Název celé objednávky: ')->setRequired('Napište název objednávky')->setMaxLength(100);
        $form->addGroup($polozkaC);
        $form->addText('popis_radky', 'Popis:')->setOption('description', 'Pokud nevyplníte, použije se název objednávky')->setMaxLength(100);
        $form->addText('firma', 'Firma (prodejce):')->setRequired('Napište název firmy.')->setMaxLength(100);
    ...
    protected function createComponentObjednavkyForm2(): Form
    {
        ...
        $form->onRender[] = '\App\Utils\FormStylePomocnik::makeBootstrap4';
        $form->addGroup('Objednávka ');
        $form->addText('popis', 'Název celé objednávky: ')->setDisabled()->setDefaultValue($posledniP)->setMaxLength(100);
        ...
        $form->addGroup($polozkaC);
        $form->addText('firma', 'Firma (prodejce):')->setDefaultValue($posledniF)->setMaxLength(100);
        $form->addText('popis_radky', 'Popis:')->setOption('description', 'Pokud nevyplníte, použije se název objednávky')->setMaxLength(100);
        $form->addSelect('cinnostVyber', 'Činnost:',$fetchedNovac )->setRequired('Vyberte prosím činnost')->setPrompt(' ');
        $form->addSelect('zakazkaVyber', 'Zakázka:',$fetchedNovaz )->setRequired('Vyberte prosím zakázku')->setPrompt(' ');
        $form->addSelect('strediskoVyber', 'Středisko:',$fetchedNovas )->setRequired('Vyberte prosím zakázku')->setPrompt(' ');
    ...
    public function dataProInsert(Form $form, $data)
    {
        ...
        $denikV = $this->database->table('denik')->where('rozpocet', $pomoc)->where('zakazky',$relevantni)
            ...
            bdump( $denikV);
            $pomocCislo =$this->database->table('cinnost')->where('id',$cinnost)->select('id_rozpocet')->fetch();
            bdump( $objednanoV+$data->castka);
